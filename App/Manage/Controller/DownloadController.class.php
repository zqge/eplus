...
class DownloadController extends CommonController {
   ...
   public function _initialize() {
    $this->modelId=C('Model_Download');//下载模型
    }
/**
* 下载列表
* @date: 2015-12-29 上午02:11:24
* @author: zhouqg
* @param: variable
* @return:
*/
    public function index(){
          ...
          // 搜索功能
   	    ...
   	    if( !empty($keyword) ){
   	    	 $where['download_title']=array('like',"%{$keyword}%");
		...
		$m = D('Download'); 
		$where['download_del']='0';
		$order='download_sort,download_addtime desc';
		$result=D('Download')->where($where)->relation(true)->select();
		//**分页实现代码
    	...
    	$result=D('Download')->where($where)->relation(true)->limit($Page->firstRow.','.$Page->listRows)->order($order)->select();
    	foreach ($result as $k => $v){
    		$result[$k]['download_title'] = Common::substr_ext($v['download_title'], 0, 16, 'utf-8',"");
    	}
//     dump($result);
//     exit;
...
*/
    public function add(){	
    	...
    	$this->assign('cate',$m);
    	//文章属性
    	$attr=M('Attr')->where(array('model_id'=>$this->modelId))->select(); //1表示新闻模型
    ...
    */
	    ...
	    $m=D('Download'); //读取Download表的model模型文件DownloadModel.class.php
    	...
    	//文件上传
    	 $pic=$_FILES['download_pic']['name'];  //缩位图
    	if (!empty($pic)) {
    		 $upload=upload_file('Download/Pic');
    		 if($upload['success']=='1'){ //文件上传成功
    		 $m->download_pic = $upload['url'];
    		 }else{ //文件上传失败
    	...
    	$file=$_FILES['download_file']['name'];  
	      	if (!empty($file)) {
    		 $upload=upload_file('Download/file','file');
    		 if($upload['success']=='1'){ //文件上传成功
    		 	$link=I('post.download_link');
    		 	if(empty($link)){
    		 	 	$m->download_link = $upload['url'];
    		 	}else{
    		 		$m->download_link = $upload['url'].','.I('post.download_link');
    		 	}
    		 }else{ //文件上传失败
    		 $this->error($upload['error']);
    		 }
    	...
    	if ($arr){
    	  //插入属性表
		   ...
		   $result=$a->addAll($data);
    		...
    		$this->success("新增成功", U('Download/index'));
    	}else {
    		$this->error('新增失败');
    	}
...
*/
    public function edit(){
    	...
    	$id=I('get.id');
    	$m=D('Download');//读取数据库模型model文件，关联模型。
    	$arr=$m->relation(true)->find($id);
		$arr['download_content_decode'] = html_entity_decode($arr['download_content']);
...
public function do_edit(){	
    	...
    	if (!empty($pic)) {
    		 $upload=upload_file('Download/pic');
    		 if($upload['success']=='1'){ //文件上传成功
    			 delete_file(I('post.d_download_pic'));  //删除被替换的文件
    			 $m->download_pic = $upload['url'];
    		 }else{ //文件上传失败
    		 $this->error($upload['error']);
    		 }
    	...
    	if ($arr){
    	  //插入属性表
		   ...
		   $result = I('post.attr');
		    foreach ($result as $k=> $v){
		 	   $data[] = array('relevance_id' => $id,'attr_id' => $v,'model_id' => $this->modelId,); 
		   	 }
		   $result=$a->addAll($data);
