...
/**
* 获取用户信息
* @date: 2015-12-31 上午01:32:19
* @author: zhm
* @param: $id:用户id ；$type 用户类型（1：用户user表；2管理员admin表）
* @return:
*/
    static public function get_user($id,$type){
    	if($type=='0'){  //游客
    			$result['name']='游客';
    			$result['image']='';
    	}else if($type=='1'){ //user表
    		
    	}else if ($type=='2'){ // admin 表
    	    	$attr=M('Admin')->where(array('id'=>$id))->find($id);
    	    	$result['name']=$attr['admin_name'];
    			$result['image']=$attr['admin_pic'];
    	}
          return $result;
    }
}