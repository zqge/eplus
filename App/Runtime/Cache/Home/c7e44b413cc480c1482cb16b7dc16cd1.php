...
<section id="content">
	<div class="wrapper">
    	...
    	<!-- navall -->
    	﻿<div id="navl" >
<span><a href="/eplus/index.php">首页</a> > 
	 <?php if(is_array($clist)): $i = 0; $__LIST__ = $clist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i;?><a href="<?php echo ($nav["url"]); ?>"><span><?php echo ($nav["column_name"]); ?></span></a>
											<?php if($i != count($clist)): ?>&gt;<?php endif; endforeach; endif; else: echo "" ;endif; ?>
</div>
      

    	<!-- /navall -->
        	<form name="formsearch"   method="post"  action="/eplus/index.php/Search/result" id="search">
        		<input type="hidden" name="kwtype" value="0" />
				<input type="text" name="keywords" id="keywords" autocomplete="off" value="在这里搜索..." required x-webkit-speech x-webkit-grammar="builtin:translate" lang="zh-CN"  />
				<input type="submit" id="submit" value="搜 索" />
        	</form>        	
    		<div class="clear"></div>
        	<article>
            	<h1 id="title"><?php echo ($v["news_title"]); ?><br />
            	<span>
            	点击数：<?php echo ($v["news_hits"]); ?> &nbsp;&nbsp; 作者：<?php echo ($v["news_author"]); ?>  &nbsp;&nbsp; 时间：<?php echo (date('Y-m-d H:i:s',$v["news_addtime"])); ?>
            	</span>
            	</h1>

            	
                <div id="info"><?php echo ($v["news_content"]); ?></div>
            </article>
      </div>
      <div class="sidebar">
        	<h3 class="cat_title">您可能感兴趣的！</h3>
          	<?php if(is_array($vlist)): $i = 0; $__LIST__ = $vlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><h5><a href="<?php echo ($v["url"]); ?>" ><?php echo ($v["news_title"]); ?></a></h5><?php endforeach; endif; else: echo "" ;endif; ?>
      </div>
      <div class="clear"></div>
  </div>
</section>
